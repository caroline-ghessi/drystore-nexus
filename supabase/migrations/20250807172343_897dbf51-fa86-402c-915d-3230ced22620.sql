...
CREATE OR REPLACE FUNCTION public.has_user_permission(user_id UUID, permission_type text)
RETURNS BOOLEAN
LANGUAGE SQL
...
AS $$
  SELECT EXISTS (
    ...
    AND user_roles.permission::text = permission_type
  );
$$;
...
SET search_path = ''
AS $$
  SELECT public.has_user_permission(auth.uid(), 'admin');
$$;
